...
case "$1" in
	rails|rake|passenger)
		if [ ! -f './config/database.yml' ]; then
			...
			# Installing DMSF
			#touch /home/redmine/.bzr.log
			touch /usr/src/redmine/log/production.log
			chmod 0666 /usr/src/redmine/log/production.log
			touch /usr/src/redmine/db/schema.rb
			chmod 0666 /usr/src/redmine/db/schema.rb
			bundle install --without development test xapian bundle
		fi
		...
		if [ "$1" != 'rake' -a -z "$REDMINE_NO_DB_MIGRATE" ]; then
			...
			gosu redmine rake redmine:plugins:migrate RAILS_ENV="production"
		fi
